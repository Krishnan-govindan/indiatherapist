...
// ─────────────────────────────────────────────────────────────
// POST /api/admin/debug/whatsapp-template
// Sends a test template message and returns raw Meta API response.
// Remove after debugging is done.
// ─────────────────────────────────────────────────────────────
router.post('/debug/whatsapp-template', async (req: Request, res: Response): Promise<void> => {
  const { to, template_name, lang } = req.body as { to?: string; template_name?: string; lang?: string };
  ...
  if (!phoneNumberId || !accessToken) {
    res.status(500).json({ error: 'META_WA env vars not set', phoneNumberId: !!phoneNumberId, accessToken: !!accessToken });
    return;
  }
  ...
  const payload = {
    ...
    type: 'template',
    template: {
      name: template_name ?? 'welcome_message',
